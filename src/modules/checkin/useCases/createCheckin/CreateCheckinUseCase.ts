...
export class CreateCheckinUseCase {
  ...
  async execute({ id, status, mealId, userId }: IRequest) {
    const meal = await this.mealRepository.findMealById(mealId);
    
    const dateNow = this.dayjsProvider.dateNow();
    const expiresDate = this.dayjsProvider.setHours(dateNow, {hours: 23, minutes: 59, seconds: 59});
    
    if (!meal) {
      throw new AppError("Meal not found");
    }
    
    const compareDays =  this.dayjsProvider.compareInDays(dateNow, meal.schedule);

    if(compareDays == 0 || compareDays > 1) {
      throw new AppError("Checkin não está disponível", 400);
    }

    await this.checkinRepository.create({ id, status, mealId, userId, expiresDate });
  }
}
